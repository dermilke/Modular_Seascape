...
merged_Atlantic <- bind_rows(
          get_mechanism_prop(datalist_Atlantic, 
                             ...
                             read.csv("output/Community_Mechanisms/Raup_Crick_Atlantic_Prok_FL.csv", row.names = 1)) %>%
          ...
          get_mechanism_prop(datalist_Atlantic, 
                             ...
                             read.csv("output/Community_Mechanisms/Raup_Crick_Atlantic_Prok_SPA.csv", row.names = 1)) %>%
          ...
          get_mechanism_prop(datalist_Atlantic, 
                             ...
                             read.csv("output/Community_Mechanisms/Raup_Crick_Atlantic_Prok_LPA.csv", row.names = 1)) %>%
...
merged_Pacific <- bind_rows(
  get_mechanism_prop(datalist_Pacific, 
                     ...
                     read.csv("output/Community_Mechanisms/Raup_Crick_Pacific_Prok_FL.csv", row.names = 1)) %>%
    mutate(Size_Fraction = "0.22-3µm"),
  get_mechanism_prop(datalist_Pacific, 
                     read.csv("output/Community_Mechanisms/Prokaryotes_weighted_bNTI_SPA_Pacific_complete.csv", row.names = 1), 
                     read.csv("output/Community_Mechanisms/Raup_Crick_Pacific_Prok_SPA.csv", row.names = 1)) %>%
    mutate(Size_Fraction = "3-8µm"),
  get_mechanism_prop(datalist_Pacific, 
                     read.csv("output/Community_Mechanisms/Prokaryotes_weighted_bNTI_LPA_Pacific_complete.csv", row.names = 1), 
                     read.csv("output/Community_Mechanisms/Raup_Crick_Pacific_Prok_LPA.csv", row.names = 1)) %>%
    mutate(Size_Fraction = ">8µm")) %>%
  mutate(Size_Fraction = ordered(Size_Fraction, levels = c("0.22-3µm", "3-8µm", ">8µm"))) %>%
...
ggsave("figs/Shaping_Mechanisms_Comparison_Combined_Epi.png", width = 5, height = 8, dpi = 300)
