...
my_tree <- ape::read.tree("data/Tree_Atlantic_Prokaryotes_NJ.tree")
...
write.csv(weighted_bNTI_FL,"output/Prokaryotes_Atlantic_weighted_bNTI_FL.csv")

weighted_bNTI_SPA <- datalist %>%
  filter_station_datalist(Size_Fraction == 3) %>%
  get_weighted_bNTI(., my_tree, boot_num = beta.reps, core_num = use_cores)

write.csv(weighted_bNTI_SPA,"output/Prokaryotes_Atlantic_weighted_bNTI_SPA.csv")

weighted_bNTI_LPA <- datalist %>%
  filter_station_datalist(Size_Fraction == 8) %>%
  get_weighted_bNTI(., my_tree, boot_num = beta.reps, core_num = use_cores)

write.csv(weighted_bNTI_LPA,"output/Prokaryotes_Atlantic_weighted_bNTI_LPA.csv")
