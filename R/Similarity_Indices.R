distance_wrapper <- function(phyloseq_obj, method = "TINA_weighted", size.thresh = 1, pseudocount = 10^-6, 
  ...
  # Main function to run for calculation of following distance-measures (after Schmidt et al. 2016)
  ...
  SparCC_wrapper <- function(phyloseq_obj, size.thresh, pseudocount,
    ...
    mat.T <- bigmemory::big.matrix(nrow=nrow(Count_Table_Filtered), ncol=nrow(Count_Table_Filtered), 
                                   dimnames = list(rownames(Count_Table_Filtered), rownames(Count_Table_Filtered)),
                                   shared=T)
    ...
    var.t <- biganalytics::colsum(mat.T);
...
fasterUniFrac <- function(o.t, tree, distance="UniFrac_unweighted", blocksize=1000, use.cores) {
  ...
  sp.o.t <- Matrix::Matrix(o.t, sparse=T, nrow = nrow(o.t), ncol = ncol(o.t),
                           dimnames = list(rownames(o.t), colnames(o.t)))
...
fast.cophenetic.phylo <- function(x, nblocks=1000, use.cores=4) {
  ...
  my.combs <- gRbase::combnPrim(tip.order, 2) %>%
    t()
